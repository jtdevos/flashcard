...
describe("StatsContext reducer", () => {
  ...
  describe("Test each case", () => {
    ...
    const getStats = (type: StatsActionType, stats?: Stats) =>
      stats
    ...
    describe("Reducer adds a new stats object when it receives a new question prompt", () => {
      ...
      const eachTest = Object.values(StatsActionType).map((actionType) => {
        ...
        return [actionType, action, initialState, exampleQuestion, result];
      });
    ...
    describe("Reducer returns correct stats", () => {
      ...
      const existingTests = Object.values(StatsActionType).map((actionType) => {
        ...
        //return the array
        return [actionType, action, existingState, result, exampleQuestion];
      });
      ...
      it.each(existingTests)(
        ...
        (actionType, action, initialState, result, question) => {
          ...
          expect(initialState[question]).toEqual(
            existingState[exampleQuestion]
          );
          //assert that the stats object at key: question matches result
          expect(reducer(initialState, action)[question]).toEqual(result);
        }
