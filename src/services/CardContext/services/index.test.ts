...
describe("getNext", () => {
  ...
  //if provided a subject, that will be on the card subject
  const getCard = (seed: string | number, subject?: string | number) => ({
    question: `${seed}`,
    answer: `${seed}!`,
  ...
  describe("show is empty", () => {
    ...
    test.each(seeds.slice(0, 3))(
      "increments current from %d",
  ...
  describe("show single subject", () => {
    ...
    it("shows only cards from the selected subject", () => {
      ...
      const cards = seeds.map((seed) =>
        //seed modulus 2 returns the remainder of deviding the seed number by 2
        ...
        seed % 2 ? getCard(seed) : getCard(seed, selectedSubject)
      );
  ...
  describe("show multiple subjects", () => {
    ...
    const show: string[] = ["firstSubject", "secondSubject", "thirdSubject"];
    ...
    const manyCards: Card[] = [...Array(21)].map((_u, seed) => {
      return seed % 3 ? getCard(seed) : getCard(seed, randomSubject());
    });

    it("shows only cards from the selected subject", () => {
      //to get the number of times to run getNext, we'll count how many cards in ManyCards
      ...
      const times = manyCards.filter((card) =>
        show.includes(card.subject)
      ).length;
      let current = 0;

      //use a for loop to asser that you always see a card with the selected subject
